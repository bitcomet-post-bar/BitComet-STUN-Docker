...
CLEANUP() {
	...
	[ "$StunModeHttps" = 0 ] || nft list chain ip STUN NAT_OUTPUT | grep -q BTTR_HTTPS || nft delete set ip STUN BTTR_HTTPS
	...
	nft list chain ip STUN BTTR_HTTP | grep -qvE '[{}]$|policy accept' || nft list chain ip STUN BTTR_UDP | grep -qvE '[{}]$|policy accept' || 	nft delete table ip STUN
